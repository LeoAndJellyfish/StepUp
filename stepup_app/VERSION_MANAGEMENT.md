...
├── scripts/                        # 🛠️ 版本管理工具
│   ├── package-all.bat            # 一键构建打包（Windows + Android）
...
### 1. 一键构建打包（推荐）
```bash
# 基本用法（自动从 pubspec.yaml 读取版本号）
.\scripts\package-all.bat

# 指定版本描述
.\scripts\package-all.bat "正式版"
.\scripts\package-all.bat "修复版"
.\scripts\package-all.bat "功能版"
.\scripts\package-all.bat "重大更新版"
```
...
使用 `package-all.bat` 脚本一键完成构建和打包：

1. **更新版本号** → 修改 `pubspec.yaml` 中的版本号
2. **运行一键打包脚本** → `.\scripts\package-all.bat "版本描述"`
3. **脚本自动完成**：
   - 从 `pubspec.yaml` 读取版本号
   - 执行 `flutter build windows --release`
   - 执行 `flutter build apk --release`
   - 分别打包 Windows 和 Android 版本
4. **验证测试** → 测试两个平台的发行包
5. **发布分享** → 分享 zip 压缩包
...
**Windows 平台**：
1. **更新版本号** → 修改 `pubspec.yaml` 中的版本
2. **构建应用** → `flutter build windows --release`
3. **打包发行** → `.\scripts\package-release.bat [版本] [描述]`
4. **验证测试** → 启动应用确认功能正常
5. **发布分享** → 分享 zip 压缩包
...
1. **更新版本号** → 修改 `pubspec.yaml` 中的版本
2. **构建应用** → `flutter build apk --release`
3. **打包发行** → `.\scripts\package-android.bat [版本] [描述]`
4. **验证测试** → 安装 APK 确认功能正常
5. **发布分享** → 分享 zip 压缩包或单独 APK 文件
...
4. **一键打包**：`package-all.bat` 自动读取版本号并构建两个平台
5. **版本追踪**：latest.txt 指向最新版本
6. **历史管理**：archive 目录保存旧版本
7. **标准命名**：统一的文件命名规范
8. **多平台支持**：Windows 和 Android 分开管理
9. **路径无关**：脚本使用相对路径，可在任何位置运行
